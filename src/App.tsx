...
import { Github, Linkedin, Mail, Code2, Database, Terminal, Award, Users, Zap, Shield, Brain, GraduationCap, MapPin, Phone, Menu, X, Download, ChevronDown } from 'lucide-react';
...
function FadeIn({ children, className = '', stagger = false }: { children: React.ReactNode; className?: string; stagger?: boolean }) {
  const ref = useFadeIn();
  return (
      ...
      <nav className="fixed top-0 w-full bg-gray-950/90 backdrop-blur-md border-b border-neon-cyan/10 z-50">
        <div className="container mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
            <a href="#" className="text-xl font-bold text-neon-cyan neon-text-cyan hover:opacity-80 transition-opacity">
              Sampat Choudhary
            </a>
            {/* Desktop nav */}
            <div className="hidden md:flex gap-8">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className={`nav-link hover:text-neon-cyan transition-colors ${activeSection === link.href.slice(1) ? 'active text-neon-cyan' : 'text-gray-400'}`}
                >
                  {link.label}
                </a>
              ))}
            ...
            <div className="flex flex-col gap-4 pt-4 pb-2">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className={`hover:text-neon-cyan transition-colors py-2 ${activeSection === link.href.slice(1) ? 'text-neon-cyan' : 'text-gray-400'}`}
        ...
        <div className="flex flex-col md:flex-row gap-12 items-center">
          ...
          <div className="md:w-2/3">
            ...
            <div className="flex flex-wrap gap-4">
              <a href="#contact" className="bg-neon-cyan text-gray-950 hover:bg-neon-cyan/80 px-8 py-3 rounded-lg font-semibold transition-all shadow-lg shadow-neon-cyan/20 hover:shadow-neon-cyan/30 hover:-translate-y-0.5">
                Get in Touch
              </a>
              ...
              <a href="#experience" className="border border-gray-700 hover:border-gray-500 px-8 py-3 rounded-lg font-medium transition-all hover:-translate-y-0.5 text-gray-300">
                View Experience
              </a>
            </div>
            {/* Scroll indicator */}
